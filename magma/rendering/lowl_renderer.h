...
typedef struct mg_lowl_renderer_init_info
{
    mg_platform *platform;
    mg_renderer_type type;
    mg_swapchain_config_info *swapchain_config_info;
}
mg_lowl_renderer_init_info;

MG_API inline void                         mg_lowl_renderer_initialize                      (mg_lowl_renderer_init_info *init_info);
MG_API inline void                         mg_lowl_renderer_shutdown                        (void);

MG_API inline void                         mg_lowl_renderer_begin_frame                     (void);
MG_API inline void                         mg_lowl_renderer_end_frame                       (void);
MG_API inline void                         mg_lowl_renderer_present_frame                   (void);

MG_API inline void                         mg_lowl_renderer_wait                            (void);

MG_API inline void                         mg_lowl_renderer_viewport                        (int32_t x, int32_t y, uint32_t width, uint32_t height);
MG_API inline void                         mg_lowl_renderer_scissor                         (int32_t x, int32_t y, uint32_t width, uint32_t height);

MG_API inline void                         mg_lowl_renderer_draw                            (uint32_t vertex_count, uint32_t first_vertex);
MG_API inline void                         mg_lowl_renderer_draw_indexed                    (uint32_t index_count, uint32_t first_index);

MG_API inline void                         mg_lowl_renderer_configure_swapchain             (mg_swapchain_config_info *config_info);

MG_API inline mg_render_pass             mg_lowl_renderer_create_render_pass              (mg_render_pass_create_info *create_info);
MG_API inline void                         mg_lowl_renderer_destroy_render_pass             (mg_render_pass render_pass);
MG_API inline void                         mg_lowl_renderer_begin_render_pass               (mg_render_pass render_pass, mg_framebuffer framebuffer, mg_render_pass_begin_info *begin_info);
MG_API inline void                         mg_lowl_renderer_begin_default_render_pass       (mg_render_pass_begin_info *begin_info);
MG_API inline void                         mg_lowl_renderer_end_render_pass                 (void);

MG_API inline mg_pipeline                mg_lowl_renderer_create_pipeline                 (mg_pipeline_create_info *create_info);
MG_API inline void                         mg_lowl_renderer_destroy_pipeline                (mg_pipeline pipeline);
MG_API inline void                         mg_lowl_renderer_bind_pipeline                   (mg_pipeline pipeline);

MG_API inline mg_vertex_buffer           mg_lowl_renderer_create_vertex_buffer            (size_t size, void *data);
MG_API inline void                         mg_lowl_renderer_destroy_vertex_buffer           (mg_vertex_buffer buffer);

MG_API inline mg_index_buffer            mg_lowl_renderer_create_index_buffer             (size_t size, void *data);
MG_API inline void                         mg_lowl_renderer_destroy_index_buffer            (mg_index_buffer buffer);

MG_API inline mg_dynamic_vertex_buffer   mg_lowl_renderer_create_dynamic_vertex_buffer    (size_t size);
MG_API inline void                         mg_lowl_renderer_destroy_dynamic_vertex_buffer   (mg_dynamic_vertex_buffer buffer);
MG_API inline void                         mg_lowl_renderer_update_dynamic_vertex_buffer    (mg_dynamic_vertex_buffer buffer, size_t size, void *data);

MG_API inline mg_dynamic_index_buffer    mg_lowl_renderer_create_dynamic_index_buffer     (size_t size);
MG_API inline void                         mg_lowl_renderer_destroy_dynamic_index_buffer    (mg_dynamic_index_buffer buffer);
MG_API inline void                         mg_lowl_renderer_update_dynamic_index_buffer     (mg_dynamic_index_buffer buffer, size_t size, void *data);

MG_API inline mg_uniform_buffer          mg_lowl_renderer_create_uniform_buffer           (size_t size);
MG_API inline void                         mg_lowl_renderer_destroy_uniform_buffer          (mg_uniform_buffer buffer);
MG_API inline void                         mg_lowl_renderer_update_uniform_buffer           (mg_uniform_buffer buffer, size_t size, void *data);

MG_API inline void                         mg_lowl_renderer_bind_vertex_buffer              (mg_vertex_buffer buffer);
MG_API inline void                         mg_lowl_renderer_bind_dynamic_vertex_buffer      (mg_dynamic_vertex_buffer buffer);
MG_API inline void                         mg_lowl_renderer_bind_index_buffer               (mg_index_buffer buffer, mg_index_type index_type);
MG_API inline void                         mg_lowl_renderer_bind_dynamic_index_buffer       (mg_index_buffer buffer, mg_index_type index_type);
MG_API inline void                         mg_lowl_renderer_bind_uniform_buffer             (mg_uniform_buffer buffer);

MG_API inline mg_image                  mg_lowl_renderer_create_image                    (mg_image_create_info *create_info);
MG_API inline void                      mg_lowl_renderer_destroy_image                   (mg_image image);
MG_API inline void                      mg_lowl_renderer_update_image                    (mg_image image, mg_image_write_info *write_info);

MG_API mg_image_array                   mg_lowl_renderer_create_image_array       (void);
MG_API void                             mg_lowl_renderer_destroy_image_array      (mg_image_array array);
MG_API void                             mg_lowl_renderer_update_image_array       (mg_image_array array, mg_image *images, mg_sampler *samplers, uint32_t count);
MG_API void                             mg_lowl_renderer_bind_image_array         (mg_image_array array);

MG_API inline mg_sampler                 mg_lowl_renderer_create_sampler                  (mg_sampler_create_info *create_info);
MG_API inline void                         mg_lowl_renderer_destroy_sampler                 (mg_sampler sampler);

MG_API inline mg_framebuffer             mg_lowl_renderer_create_framebuffer              (mg_framebuffer_create_info *create_info);
MG_API inline void                         mg_lowl_renderer_destroy_framebuffer             (mg_framebuffer framebuffer);

MG_API inline void                         mg_lowl_renderer_push_constants                  (uint32_t size, void *data);
